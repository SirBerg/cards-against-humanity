...
//this function takes a gameID and a userID and updates their cards.ts in such a way that they at most have 5 cards.ts
export function drawWhiteCard(gameID:string, userID:string, games:gamesType, memoryCards:any, replaceCard?:string):Array<clientCard> {
    function getRandomWhiteCard(): clientCard {
        ...
        //make sure there are cards.ts in this pack, else return immediately
    ...
    //check if the user has 5 cards.ts already or greater
    if (returnCards.length >= 5 && !replaceCard) {
        //remove the cards.ts that are to much
    ...
    //draw as many cards.ts as we can without going over the limit
    while (returnCards.length < 5) {
